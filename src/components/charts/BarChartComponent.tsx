...
export const BarChartComponent: React.FC<Props> = ({ data, config }) => {
        ...
        >
          ...
          {config.yAxisKeys.map((yKey, index) => (
            <Bar
              ...
              radius={isHorizontal ? [0, 4, 4, 0] : isStacked ? [0, 0, 0, 0] : [4, 4, 0, 0]}
            >
              {config.yAxisKeys.length === 1 && !isStacked && data.map((_, cellIdx) => (
                <Cell
                  key={`cell-${cellIdx}`}
                  fill={palette[cellIdx % palette.length]}
                />
              ))}
            </Bar>
          ))}
